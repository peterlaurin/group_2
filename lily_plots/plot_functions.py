...
def gender_by_country(database_name, author_min, create_html = False, show_graph = True):
    """
    ...
    if create_html:
        fig.write_html('bar_countryByGender_' + str(author_min) + '.html', include_plotlyjs = False) #html code
    fig.show()

def gender_by_rank(database_name, create_html = False, show_graph = True):
    """
...
def gender_by_field(database_name, create_html = False, show_graph = True):
    """
    ...
    gender_by_field_individual(database_name, field_pcts, create_html, show_graph)
